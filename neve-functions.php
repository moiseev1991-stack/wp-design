...
// Allow anchor links in manual excerpts
...
// Disable wpautop on front page to preserve hero HTML
...
// Append latest 3 posts cards on front page
add_filter('the_content', function($content) {
    ...
    $html = '<section class="hp-latest-posts"><div class="hp-latest-posts__head"><h2>Ostatnie artykuły</h2><a href="/blog/">Zobacz wszystkie →</a></div><div class="hp-latest-posts__grid">';

    foreach ($posts as $p) {
        $url     = get_permalink($p->ID);
        $title   = esc_html($p->post_title);
        $excerpt = $p->post_excerpt ?: wp_trim_words($p->post_content, 22, '…');
        $thumb   = get_the_post_thumbnail($p->ID, 'medium_large', ['alt' => $title]);
        $date    = get_the_date('j F Y', $p->ID);

        $html .= '<article class="hp-card"><a href="' . $url . '" class="hp-card__img-wrap">' . $thumb . '</a><div class="hp-card__body"><h3 class="hp-card__title"><a href="' . $url . '">' . $title . '</a></h3><time class="hp-card__date">' . $date . '</time><p class="hp-card__excerpt">' . wp_kses($excerpt, ["a"=>["href"=>[],"target"=>[],"rel"=>[]]]) . '</p><a href="' . $url . '" class="hp-card__more">Czytaj dalej →</a></div></article>';
    }
